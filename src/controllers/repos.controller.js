...
/** Extract the GitHub user ID from the request JWT (returns null if absent/invalid) */
function getGithubIdFromReq(req) {
    try {
        const token = req.cookies?.token;
        if (!token) return null;
        const decoded = jwt.verify(token, JWT_SECRET);
        return decoded.githubId || null;
    } catch {
        return null;
    }
}
...
exports.trackRepo = async (req, res) => {
    ...
    try {
        ...
        // Also register the repo in the user's repository list so it shows up in the sidebar
        try {
            const userGithubId = getGithubIdFromReq(req);
            if (userGithubId) {
                const existingUser = await db.getUserByGithubId(userGithubId, req);
                const currentRepos = existingUser?.repositories || [];
                if (!currentRepos.includes(ghRepo.id)) {
                    await db.updateUser(userGithubId, {
                        repositories: [...currentRepos, ghRepo.id],
                    });
                }
            }
        } catch (userErr) {
            console.warn("[trackRepo] Could not update user repositories list:", userErr.message);
        }
...
// Background helper — runs without blocking the HTTP response
async function upsertPRsToDb(ghPrs, owner, name, token) {
    try {
        const fullName = `${owner}/${name}`;
        let repo;
        try {
            repo = await db.getRepoByFullName(fullName);
        } catch (e) {
            if (e.status === 404) {
                // Repo not in DB yet (was imported via old flow) — create it now
                try {
                    const ghRepo = await github.getRepo(owner, name, token);
                    repo = await db.createRepo({
                        githubId: ghRepo.id,
                    ...
                    console.warn("[upsertPRsToDb] Could not auto-create repo:", createErr.message);
                    return;
                }
            } else {
                throw e;
        ...
        for (const ghPr of ghPrs) {
            ...
            try {
                ...
                // Ignore other errors per-PR so one failure doesn't abort the rest
            }
        }
    } catch (err) {
        console.warn("[upsertPRsToDb] Background sync error:", err.message);
    }
}
...
exports.listRepoPulls = async (req, res) => {
    try {
        ...
        // Fire-and-forget: persist/update PRs in MongoDB without blocking the response
        upsertPRsToDb(prs, owner, name, token).catch(() => { });
    } catch (err) {
        res.status(err.status || 500).json({ error: err.message });
    }
};
